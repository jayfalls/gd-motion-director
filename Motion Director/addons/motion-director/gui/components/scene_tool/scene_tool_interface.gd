...
var controller: MotionDirectorController
...
## Constants
const controller_name: String = "MotionDirectorController"
### Interface Groups
...
func _on_none_detected_create_button_pressed():
	if not is_instance_valid(edited_scene_root):
		return
	var node: MotionDirectorController = MotionDirectorController.new()
